...
async function runReadmeCapture() {
  try {
    ...
    const updateShowcase = await mainWindow.webContents.executeJavaScript(`window.__starbrowserTest?.showUpdateShowcase()`)
    if (!updateShowcase?.visible) throw new Error(`更新浮窗未完整显示：${JSON.stringify(updateShowcase)}`)
    await delay(700)
    await capture('03-update.png')
  ...
  try {
    ...
    const updateUi = await mainWindow.webContents.executeJavaScript(`(async () => {
      ...
      const result = {
        ...
        versionShown: text.includes('v1.7.0') && text.includes('v9.9.9'),
